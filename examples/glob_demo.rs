...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    demo_pattern(r"file\*.txt", &[
        ("file*.txt", true),
        ("file1.txt", false),
        ("fileany.txt", false),
    ], MatchMode::CaseSensitive)?;

    demo_pattern(r"test\?.log", &[
        ("test?.log", true),
        ("test1.log", false),
    ], MatchMode::CaseSensitive)?;
...
fn demo_pattern(
    ...
    for (text, expected) in tests {
        ...
        println!("  {} \"{}\" → {}", status, text, if result { "MATCH" } else { "no match" });
    }
...
fn performance_demo() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (name, pattern_str) in &patterns {
        ...
        println!("  5000 matches: {:?} ({:.2} µs/match)", 
                 match_time, 
    ...
    for multiplier in [1, 10, 100, 1000] {
        ...
        println!("    100 matches: {:?} ({:.2} µs/match)", 
                 elapsed, 
