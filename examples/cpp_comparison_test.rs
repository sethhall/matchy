...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test 1: 10K patterns, 20K queries (matching C++ test_performance_benchmark)
    {
        ...
        if build_ms > MAX_BUILD_MS {
            println!("  FAIL: Build took {} ms (expected < {} ms)", build_ms, MAX_BUILD_MS);
            std::process::exit(1);
        }
        ...
        if query_ms > MAX_TIME_MS {
            println!("  FAIL: Queries took {} ms (expected < {} ms)", query_ms, MAX_TIME_MS);
        ...
        if queries_per_second < MIN_QPS {
            println!("  FAIL: Query throughput too low: {} qps", queries_per_second);
        ...
        println!("        - Time: {} ms (limit: {} ms)", query_ms, MAX_TIME_MS);
        println!("        - Throughput: {} qps (min: {} qps)", queries_per_second, MIN_QPS);
    }
    ...
    println!("\nLarge Pattern Set: 50K patterns");
    {
        ...
        if build_ms > MAX_LARGE_BUILD_MS {
            println!("  FAIL: Build took {} ms (expected < {} ms)", build_ms, MAX_LARGE_BUILD_MS);
            std::process::exit(1);
        }
        ...
        println!("        - Throughput: {} qps (min: {} qps)", queries_per_second, MIN_QPS_LARGE);
    }
