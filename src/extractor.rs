...
impl PatternExtractor {
    ...
    fn is_valid_domain(&self, line: &[u8], span: (usize, usize)) -> bool {
        ...
        if labels.len() < self.config.min_domain_labels {
            return false;
        }
    ...
    /// Extract IPv4 addresses using fast byte scanning
    fn extract_ipv4<'a>(
        &self,
        line: &'a [u8],
        matches: &mut Vec<Match<'a>>,
        ...
        while i < line.len() {
            ...
            // Try to parse an IPv4 address starting here
            if let Some((ip, end)) = self.try_parse_ipv4(line, i) {
                matches.push(Match {
    ...
    /// Extract email addresses by finding @ anchors
    fn extract_emails<'a>(
        &mut self,
        line: &'a [u8],
        matches: &mut Vec<Match<'a>>,
        ...
        while i < line.len() {
            ...
            // Found @, try to extract email
            if let Some(email_span) = self.extract_email_at(line, i) {
                // Validate UTF-8
...
fn is_word_boundary(b: u8) -> bool {
    b.is_ascii_whitespace() || matches!(b, b'/' | b',' | b';' | b':' | b'(' | b')' | b'[' | b']' | b'{' | b'}' | b'<' | b'>' | b'"' | b'\'')
}
...
mod tests {
    ...
    fn test_domain_extraction_log_line() {
        ...
        // Realistic log line
        let line = b"2024-01-15 10:32:45 GET /api evil.example.com 192.168.1.1 - malware.badsite.org";
        let matches = extractor.extract_from_line(line);
        
        assert!(matches.len() >= 2);
        // Should find both domains
        let domains: Vec<&str> = matches.iter()
    ...
    fn test_ipv4_extraction_basic() {
        ...
        // Should find the IP
        let ips: Vec<Ipv4Addr> = matches.iter()
            .filter_map(|m| match m.item {
    ...
    fn test_email_extraction_basic() {
        ...
        let emails: Vec<&str> = matches.iter()
            .filter_map(|m| match m.item {
